...
#X_T = np.hstack((np.ones((30,1)), wine_T[1:31,1:14].astype(np.float)))
#y_T = wine_T[1:31,0].astype(np.float)
#clf_SVC = SVC(kernel='linear')
        
clf_Ridge = linear_model.Ridge(alpha = 3.844, fit_intercept = False)
clf_Ridge.fit(X1, y1)
        #coeffs = clf_Lasso.coef_
loss = (1-clf_Ridge.score(X1, y1))*np.var(y1)  #normalized L2-error
        #err = ((clf_Lasso.predict(X1)-y1)**2).sum()/4    # returns the same times N
class_acc = 1-np.mean((np.sign(clf_Ridge.predict(X1))-y1)**2)/4
...
y_test2 = np.sign(clf_Ridge.predict(X_T))
