...
clf_Ridge = []                # list of linear models computed by boosting
...
for i in range(10):     # do boosting 10 times       
    clf_Ridge.append(linear_model.Ridge(alpha = 3.844, fit_intercept = False))
    clf_Ridge[i].fit(X1, y_cur)
    
    y_trai += clf_Ridge[i].predict(X1)  # best prediction after i boosting steps  
    class_err = 1-np.mean((np.sign(y_trai)-y1)**2)/4 # accuracy after i boosting steps  
        
    print("   classification accuracy: "),
    print(class_err)   

    y_cur  -= clf_Ridge[i].predict(X1)  #subtract predictions
         # during next boosting step, the differences y_cur need to be predicted
    y_pred += clf_Ridge[i].predict(X_T) #compute predictions for test data
